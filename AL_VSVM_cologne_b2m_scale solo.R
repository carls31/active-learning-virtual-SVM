...
for(realization in seq(along = c(1:nR))){#}
  ...
  for(sample_size in seq(along = c(1:length(sampleSizePor)))){#}
    ...
    classSize = c(round(min(table(trainDataCurRemaining$REF))/10))# number of samples for each class # 250, 500, 750, 1000, 1500, 3000, 5803 for multiclass # min(table(trainDataCurRemaining_it$REF))
