...
resampledSize = c(2*b,b)        # total number of relabeld samples # b, b*2, b*6
newSizes = c(4,2)           # number of samples picked in each Active Learning iteration # 4, 5, 10, 20, resampledSize
...
colheader = as.character(clusterSizes)                 # corresponding column names    
...
for(realization in seq(along = c(1:nR))){#}
  ...
  for(sample_size in seq(along = c(1:length(sampleSizePor)))){#}
    ...
    for(cS in 1:length(clusterSizes)){
      ...
      AccuracySVM[realization,cS] = as.numeric(accSVM$overall["Accuracy"])
      AccuracySVM_M[realization,cS] = as.numeric(accSVM_M$overall["Accuracy"])
      AccuracySVM_SL_Un_b[realization,cS] = as.numeric(accSVM_SL_Un_b$overall["Accuracy"])
      
      AccuracyVSVM[realization,cS] = as.numeric(accVSVM$overall["Accuracy"])
      AccuracyVSVM_SL[realization,cS] = as.numeric(accVSVM_SL$overall["Accuracy"])
      AccuracyVSVM_SL_Un_b[realization,cS] = as.numeric(accVSVM_SL_Un_b$overall["Accuracy"])
      
      AccuracyVSVM_SL_vUn_b[realization,cS] = as.numeric(accVSVM_SL_vUn_b$overall["Accuracy"])
      
      AccuracyVSVM_SL_Un_it[realization,cS] = as.numeric(accVSVM_SL_Un_it$overall["Accuracy"])
      
      # ******** KAPPA SCORE
      KappaSVM[realization,cS] = as.numeric(accSVM$overall["Kappa"])
      KappaSVM_M[realization,cS] = as.numeric(accSVM_M$overall["Kappa"])
      KappaSVM_SL_Un_b[realization,cS] = as.numeric(accSVM_SL_Un_b$overall["Kappa"])
      
      KappaVSVM[realization,cS] = as.numeric(accVSVM$overall["Kappa"])
      KappaVSVM_SL[realization,cS] = as.numeric(accVSVM_SL$overall["Kappa"])
      KappaVSVM_SL_Un_b[realization,cS] = as.numeric(accVSVM_SL_Un_b$overall["Kappa"])
      
      KappaVSVM_SL_vUn_b[realization,cS] = as.numeric(accVSVM_SL_vUn_b$overall["Kappa"])
      
      KappaVSVM_SL_Un_it[realization,cS] = as.numeric(accVSVM_SL_Un_it$overall["Kappa"])
    }
...
if(length(clusterSizes)>=2){
  ...
  save(KappaSVM,KappaSVM_M,KappaSVM_SL_Un_b,KappaVSVM,KappaVSVM_SL,KappaVSVM_SL_Un_b,KappaVSVM_SL_vUn_b,KappaVSVM_SL_Un_it,
       file=paste0(format(Sys.time(),"%Y%m%d_%H%M"),"_cluster_",city,"_",invariance,"_",model_prob,"_Kappa_",b,"Unl_",nR,"nR_",length(sampleSizePor),"SizePor.RData"))
