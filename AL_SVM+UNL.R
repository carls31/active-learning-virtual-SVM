...
resampledSize = c(3*b,6*b)    # total number of relabeled samples # b, 2*b, 3*b, 6*b
newSizes = c(b/2)            # number of samples picked in each Active Learning iteration # 4, 5, 10, 20, resampledSize
classSize = c(10*b)  #5*b   # number of samples for each class # 25, 50, 75, 100, 150, 300, 580 for multiclass # round(min(600,min(table(trainDataCurRemaining$REF)))/10)
clusterSizes = c(2*b,6*b)       # number of clusters used to pick samples from different groups # 40, 60, 80, 100, 120, 300
  ...
  if (file.exists(model_name) && !train) {
    ...
    for(jj in 1:length(bound)){
      ...
      for (kk in seq(along = c(1:length(boundMargin)))){
        ...
        if(actKappa < tunedVSVM$resample$Kappa){ print(paste("current best kappa:",round(tunedVSVM$resample$Kappa,4),"| training time:",t.time,""))
  ...
  for(city in cities){
    for(invariance in invariances){
      ...
      for(realization in c(1:nR)){#}
        ...
        print(paste0("CPU cores: ",num_cores," | seed: ",seed)) 
        ...
        for(sample_size in c(1:length(sampleSizePor))){#}
          ...
          print(paste0("SVM accuracy: ",round(accSVM$overall["Accuracy"],5)," | training time: ",train.time,""))
          gc()
          ...
          print(paste0("SVM_M accuracy: ",round(accSVM_M$overall["Accuracy"],5)," | training time: ",train.time,""))
          gc()
          ...
          print(paste0("VSVM accuracy: ",round(accVSVM$overall["Accuracy"],5)," | training time: ",train.time,""))
          gc()
          ...
          ###################################### AL_SVM+UNL #######################################
          model_name_AL_SVMUNL = paste0(format(Sys.time(),"%Y%m%d"),"AL_SVM+UNL_",city,"_",invariance,"_",model_prob,"_",sampleSizePor[sample_size],"Size_",b,"Unl_",seed,"seed.rds")
          if(num_cores>=16){
            ...
            # **********************
            # get original SVs index of base SVM
            SVindex_ud = new_tunedSVM$finalModel@SVindex  
            ...
            for(clS in 1:length(classSize)){
              ...
              # **********************
              # get original SVs of base SVM
              SVtotal_ud = samplesRemaining[ ,c(sindexSVMDATA:eindexSVMDATA,ncol(trainDataCur))]
              
              REF_ud = predict(new_tunedSVM, SVtotal_ud[,-ncol(SVtotal_ud)])
              
              # get SV of unlabeled samples
              index_ud = 1:nrow(samplesRemaining)  
              total_ud = samplesRemaining[index_ud ,c(sindexSVMDATA:eindexSVMDATA)] 
              total_ud = cbind(total_ud, REF_ud)
              
              if(city=="cologne"){
                if(invariance=="scale"){ 
                  upd_SLresult <- self_learn(testFeatsub, testLabels, bound, boundMargin, model_name_AL_SVMUNL, SVtotal_ud, objInfoNames,rem_extrem,rem_extrem_kerneldist, #classProb=TRUE,
                                                 SVL_variables = list(
                                                   list(total_ud, SVL2=setNames( cbind(samplesRemaining[c((sindexSVMDATA - 2*numFeat):(sindexSVMDATA - numFeat - 1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL3=setNames( cbind(samplesRemaining[c((sindexSVMDATA - numFeat):(sindexSVMDATA -1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL5=setNames( cbind(samplesRemaining[c((sindexSVMDATA + numFeat):((sindexSVMDATA + 2*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL6=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 2*numFeat):((sindexSVMDATA + 3*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL7=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 3*numFeat):((sindexSVMDATA + 4*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL8=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 4*numFeat):((sindexSVMDATA + 5*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL9=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 5*numFeat):((sindexSVMDATA + 6*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL10=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 6*numFeat):((sindexSVMDATA + 7*numFeat)-1))],REF_ud),objInfoNames)),
                                                   list(total_ud, SVL11=setNames( cbind(samplesRemaining[c((sindexSVMDATA + 7*numFeat):((sindexSVMDATA + 8*numFeat)-1))],REF_ud),objInfoNames))
                                                 )
                  )
                }else{
                  upd_SLresult <- self_learn(testFeatsub, testLabels, bound, boundMargin, model_name_AL_SVMUNL, SVtotal_ud, objInfoNames,rem_extrem,rem_extrem_kerneldist, #classProb=TRUE,
                                                 SVL_variables = list(
                                                   list(total_ud, S01C09=setNames( cbind(samplesRemaining[c((numFeat+1):(2*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S03C05=setNames( cbind(samplesRemaining[c(((2*numFeat)+1):(3*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S03C07=setNames( cbind(samplesRemaining[c(((3*numFeat)+1):(4*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S05C03=setNames( cbind(samplesRemaining[c(((4*numFeat)+1):(5*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S05C05=setNames( cbind(samplesRemaining[c(((5*numFeat)+1):(6*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S05C07=setNames( cbind(samplesRemaining[c(((6*numFeat)+1):(7*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S07C03=setNames( cbind(samplesRemaining[c(((7*numFeat)+1):(8*numFeat))],REF_ud),objInfoNames)),
                                                   list(total_ud, S09C01=setNames( cbind(samplesRemaining[c(((8*numFeat)+1):(9*numFeat))],REF_ud),objInfoNames))
                                                 )
                  )
                }
              }else{
              ...
              new_tunedSVM <- upd_SLresult$bestFittingModel
              ...
              for(cS in 1:length(clusterSizes)){
                for(rS in 1:length(resampledSize)){
                  for(nS4it in 1:length(newSizes)){
                    ...
                    upd_trainDataCurFeatsub <- upd_SLresult$best_trainFeatVSVM
                    upd_trainDataCurLabels <- upd_SLresult$best_trainLabelsVSVM
                    ...
                    for (iter in 1:num_iters){
                      ...
                      predLabelsVSVM = predict(new_tunedSVM, upd_trainDataCurFeatsub)
                      ...
                      if(model_prob=="binary"){sampled_data <- margin_sampling(new_tunedSVM, predLabelsVSVM_unc,pred_one)
                      }else{sampled_data <- mclu_sampling(new_tunedSVM, predLabelsVSVM_unc)}
            ...
            print(paste0("VSVM_SL - AL accuracy: ",round(accVSVM_SL_itAL$overall["Accuracy"],5)," | training time: ",train.time,""))
            if(actAcc>best_acc){ 
              best_acc <- actAcc
              best_model <- model_name_AL_SVMUNL
            }
            gc()
        ...
        if(realization==3 && sample_size==4){
          ...
          saveRDS(new_tunedSVM, model_name_AL_SVMUNL)
        }
      }
