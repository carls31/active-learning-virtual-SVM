script = "ALTSLv1"  # -> New train samples are Active labeled samples
#####################################################  Libraries  ################################################
library(caret)
library(kernlab)
library(sampling)
    ...
    for (city in cities) {
      ...
      for (realization in seq(1,nR)) {
        ...
        for (sample_size in seq(1, length(sampleSizePor), by=2)) {
          ...
          cat(city," ",model_prob ," ",invariance," | realization [",realization,"/",nR,"] | labeled samples: ",sampleSizePor[sample_size]," [",(sample_size+1)/2,"/",round((length(sampleSizePor)+1)/2),"] | seed: ",seed,"\n",sep="")
          ...
          tr.timeSVM_SL_Un[realization,sample_size] = t.timeSVMUn
          ...
          tr.timeVSVM_SL[realization,sample_size] = t.timeSL
          ...
          bb = 1 # for (bb in seq(along=b)) {
            ...
            tr.timeVSVM_SL_Un[realization,sample_size] = trainUn.time
          ...
          cat("\n") ################################# VSVM-SL + virtual semi-labeled samples ##################################
            ...
            cat("evaluation of VSVM SL with ",b[bb]," virtual semi-labeled samples | ",sampleSizePor[sample_size]," [",(sample_size+1)/2,"/",round((length(sampleSizePor)+1)/2),"]\n",sep="") #  [",bb,"/",length(b),"]","
            if (invariance=="scale") {
              SVL_variables = list(
                list(SVtotal, SVL2),
                list(SVtotal, SVL3),
          ...
          tr.timeVSVM_SL_vUn[realization,sample_size] = trainvUn.time
          ...
          if (num_cores>=10 && sample_size<length(sampleSizePor)) {
            ...
            cat("\n") ############################### Random AL SVM #######################################
              ...
              cat("random active labeling | ",sampleSizePor[sample_size]," [",(sample_size+1)/2,"/",round(length(sampleSizePor)/2),"]\n",sep="")
                    ...
                    cat("adding ",newSize," random active samples | pool size: ",
                    ...
                    # rand_newSize=c(ceiling(newSize/nclass),ceiling(newSize/nclass),ceiling(newSize/nclass),ceiling(newSize/nclass),ceiling(newSize/nclass),ceiling(newSize/nclass))
                    randStratSampRemaining = strata(upd_dataCur, size = newSize, method = "srswor") # c("REF"),
                    ...
                    train.time <- round(as.numeric((Sys.time() - trainStart.time), units = "secs")+sampling.time, 1)
                    tmp_pred = predict(tmp_new_tunedSVM_r2, validateFeatsub)
                    accVSVM_SL_AL_random  = confusionMatrix(tmp_pred, validateLabels)
 
            cat(model_name_AL_VSVMSL_r, " accuracy: ",round(accVSVM_SL_AL_random$overall["Accuracy"],5)," | execution time: ",train.time,"sec\n",sep="")
            ...
            cat("active labeling ",model_name_AL_VSVMSL," | ",length(trainLabels_AL)," [",(sample_size+1)/2,"/",round(length(sampleSizePor)/2),"] | [",realization,"/",nR,"]\n",sep="")
                    ...
                    trainStart.time <- Sys.time()
                      ...
                      if (model_prob=="binary") {
                        sampled_data <- margin_sampling(tmp_new_tunedSVM, predLabelsVSVM_unc, pred_one, binaryClassProblem, plot_flag = model_name_AL_VSVMSL)
                      } else {
                        sampled_data <- mclu_sampling(  tmp_new_tunedSVM, predLabelsVSVM_unc, pred_all, binaryClassProblem, plot_flag = model_name_AL_VSVMSL) 
                        }
                      ...
                      result <- add_AL_samples(sampled_data,
                                               ...
                                               upd_dataCur$ID_unit, flag_cluster=TRUE)
            ...
            cat(model_name_AL_VSVMSL, " accuracy: ",round(accVSVM_SL_itAL,5)," | execution time: ",train.timeALv1_tSNE_VSVMSL,"sec\n",sep="")
            ...
            tr.timeVSVM_SL_Un_it[realization,sample_size+1] = train.timeALv1_tSNE_VSVMSL
            ...
            tr.timeVSVM_SL_Un_itSL2[realization,sample_size+1] = train.timeALv2_tSNE_VSVMSL
            ...
            tr.timeVSVM_SL_Un_itTSL2[realization,sample_size+1] = train.timeALv2_SEMI_VSVMSL
          ...
          if (realization==1 && sample_size==5) {
            ...
            saveRDS(model_name_ALTrainSL_VSVMSL2, paste0(format(Sys.time(),"%Y%m%d"),model_name_ALTrainSL_VSVMSL,"_",city,"_",model_prob,"_",invariance,"_",script,"_",sampleSizePor[sample_size],"sampleSizePor_",b,"Unl_",seed,"seed.rds"))
            saveRDS(model_name_ALTrainSL_VSVMSL, paste0(format(Sys.time(),"%Y%m%d"),model_name_ALTrainSL_VSVMSL,"_",city,"_",model_prob,"_",invariance,"_",script,"_",sampleSizePor[sample_size],"sampleSizePor_",b,"Unl_",seed,"seed.rds"))
          }
        }
        cat("\n") ################################### End realization ############################################
      ...
      if (length(sampleSizePor)>=4) {
        ...
        save(AccuracySVM, AccuracySVM_SL_Un,AccuracyVSVM_SL,AccuracyVSVM_SL_Un,AccuracyVSVM_SL_vUn,
             AccuracyVSVM_SL_Un_it,AccuracyVSVM_SL_Un_random_it,AccuracyVSVM_SL_Un_itSL,AccuracyVSVM_SL_Un_itTSL,AccuracyVSVM_SL_Un_itSL2,AccuracyVSVM_SL_Un_itTSL2,
        ...
        save(KappaSVM, KappaSVM_SL_Un,KappaVSVM_SL,KappaVSVM_SL_Un,KappaVSVM_SL_vUn,
             KappaVSVM_SL_Un_it,KappaVSVM_SL_Un_random_it,KappaVSVM_SL_Un_itSL,KappaVSVM_SL_Un_itTSL,KappaVSVM_SL_Un_itSL2,KappaVSVM_SL_Un_itTSL2,
