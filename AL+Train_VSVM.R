      ...
      for (realization in seq(nR)) {#}
        ...
        for (sample_size in seq(along=sampleSizePor)) {#}
          ...
          if (num_cores>=4) {
            ...
            AccuracyVSVM_SL_Un_it_T[realization,sample_size] = as.numeric(accVSVM_SL_itAL_T$overall["Accuracy"])
            KappaVSVM_SL_Un_it_T[realization,sample_size] = as.numeric(accVSVM_SL_itAL_T$overall["Kappa"])
            if(sample_size==4 && actAcc>best_acc){ 
              best_acc <- actAcc 
