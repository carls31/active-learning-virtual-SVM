...
dbf <- read.dbf("./shape_files/cologne/cologne_frame.dbf")
...
####+++++++++++++++ here we need to include col and hag in one dbf file
  ...
  new.dbf <- cbind(new.dbf,name=read.csv2( )[,2])
  
  #create column name
  colnames(new.dbf) <- gsub("[a-z_]*","",gsub("_[0-9]*[a-z]*.csv$","",gsub("^.+/","",ifelse(grepl("Shape",col[i]),sub("Shape","SHape",col[i]),sub("Scale","SCale",col[i])))))



###### be careful!!! a .dbf file cannot have attribute names longer than 10 characters.
# it will be cut after the 10th character without a warning when writing  
write.dbf(new.dbf,"./shape_files/cologne/final_cologne_frame.dbf",factor2char = F)
...
# use gdal to rasterize extremly fast
  ...
  writeRaster(ref.raster,paste0("./raster/cologne/",colnames(new.dbf),".tif"),overwrite=TRUE)
  gdal_rasterize(src_datasource ="./shape_files/cologne/final_cologne_frame.shp",
                 dst_filename = paste0("./raster/cologne/",colnames(new.dbf),".tif"),
                 a=colnames(new.dbf))
